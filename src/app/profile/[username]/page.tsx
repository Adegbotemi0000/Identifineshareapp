...
import { getProfile, getPortfolioMeta, type Profile } from "@/lib/storage";
...
const SOCIAL_ICONS: { key: keyof Profile; label: string; icon: IconComponent }[] = [
  { key: "linkedin", label: "LinkedIn", icon: LinkedinIcon },
  { key: "instagram", label: "Instagram", icon: InstagramIcon },
  { key: "twitter", label: "Twitter / X", icon: TwitterXIcon },
  { key: "facebook", label: "Facebook", icon: FacebookIcon },
  { key: "linktree", label: "Linktree", icon: Link2 },
  { key: "customLink1", label: "Link", icon: Link2 },
  { key: "customLink2", label: "Link", icon: Link2 },
];
...
export default function PublicProfilePage() {
  ...
  useEffect(() => {
    ...
    if (stored && stored.username.toLowerCase() === params.username?.toLowerCase()) {
      setProfile(stored);
  ...
  async function handleShare() {
    ...
    if (navigator.share) {
      try {
        await navigator.share({
          ...
          text: `Check out ${fullName || profile.username}'s IdentiShare profile`,
          url,
        });
      } catch {
        // User cancelled the share sheet — not an error, per spec §28.
      }
      return;
    }
    ...
    try {
      ...
      // Clipboard unavailable — fail quietly, per spec §28.
    }
  }
      ...
      <PhoneViewport>
        <div className="px-8 py-10 h-full flex flex-col items-center justify-center text-center gap-4">
          <p className="text-sm text-ink-soft">
            This profile isn&apos;t available. It may not exist, or it belongs
            to a different browser session in this prototype.
          </p>
          <Link href="/" className="text-sm text-gold font-medium">
            Back to IdentiShare
          </Link>
        </div>
      </PhoneViewport>
  ...
  const filledSocials = SOCIAL_ICONS.filter((s) => Boolean(profile[s.key]));
  const ownerEmail = profile.officialEmail || profile.personalEmail;

  const backgroundColor = profile.backgroundColor || "#fefdfb";
  const accentColor = profile.accentColor || "#ad8a3f";
  const pageTextColor = getContrastTextColor(backgroundColor);
  const accentForeground = getContrastTextColor(accentColor);
  ...
  const themeStyle: ThemeStyle = {
    ...
    "--color-ink-soft":
      pageTextColor === "#ffffff" ? "rgba(255,255,255,0.72)" : "rgba(23,21,15,0.65)",
    ...
    <div style={themeStyle}>
      <PhoneViewport>
        <div className="min-h-full relative flex flex-col pb-10">
          ...
          <div className="relative z-10 px-6 -mt-14 text-center">
            ...
            {profile.bio && (
              <p className="mt-4 text-sm text-ink-soft leading-relaxed">{profile.bio}</p>
            )}
            ...
            <div className="mt-5 flex gap-3">
              ...
              <button
                ...
                style={{ borderColor: accentColor, color: pageTextColor }}
              >
            ...
            {filledSocials.length > 0 && (
              <div className="mt-7 flex items-center justify-center flex-wrap gap-3">
                {filledSocials.map((s) => (
                  <a
                    key={s.key}
                    href={withProtocol(profile[s.key] as string)}
                    target="_blank"
                  ...
                  >
                    <s.icon className="w-5 h-5" style={{ color: accentForeground }} />
                  </a>
                ))}
              </div>
            )}
              ...
              <div className="mt-8 text-left">
                ...
                <div className="flex items-center gap-2.5 rounded-xl border border-gold-line bg-paper px-4 py-3">
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="text-sm text-ink truncate">{portfolioMeta.name}</p>
            ...
            <div className="mt-8 rounded-2xl border border-gold-line bg-ivory px-5 py-6 text-center">
              <p className="text-xs text-ink-soft">Like what you see?</p>
      ...
      {ownerEmail && (
        <ExchangeContactModal
          ...
          ownerName={fullName || profile.username}
          ownerEmail={ownerEmail}
        />
      )}

      <ImageLightbox
        src={lightboxIndex !== null ? profile.workImages[lightboxIndex] : null}
        alt={`Image ${(lightboxIndex ?? 0) + 1}`}
        downloadName={`${profile.username}-image-${(lightboxIndex ?? 0) + 1}.jpg`}
